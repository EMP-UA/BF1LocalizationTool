...
//
//     Призначення — НЕ повний компілятор Lua-джерела (ми не пишемо
//     лексер/парсер/кодогенератор довільного .lua тексту). Призначення —
...
//
//     Purpose — NOT a full Lua-source compiler (we do not write a
//     lexer/parser/codegen for arbitrary .lua text). Purpose — to allow
...
public static class Lua50BytecodeWriter
{
    ...
    //     in both Bx and SBx=Bx-MAXARG_sBx together) — so we encode
