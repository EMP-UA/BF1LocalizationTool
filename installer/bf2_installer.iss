; ==============================================================================
; bf2_installer.iss
; UA: Інсталятор української локалізації для Star Wars: Battlefront II
;     (Classic, 2005). Окремий скрипт для Star Wars: Battlefront
;     (Classic, 2004): bf1_installer.iss.
; EN: Ukrainian localization installer for Star Wars: Battlefront II
;     (Classic, 2005). Separate script for Star Wars: Battlefront
;     (Classic, 2004): bf1_installer.iss.
...
#define AppId "{{44092040-6C5D-44DC-B8C9-A6AE7136E0C3}}"
; Офіційний ID гри у Steam
#define SteamAppId "6060"
#define GroupName "Star Wars Battlefront II (Classic, 2005) Українізатор"
...
OutputDir=Output
OutputBaseFilename=SWB2_UA_v{#AppVersion}
...
[Messages]
ukrainian.SelectDirDesc=Виберіть теку, у якій встановлено Star Wars: Battlefront II (Classic, 2005).
ukrainian.SelectDirLabel3=Інсталятор встановить локалізацію у відповідну теку.
english.SelectDirDesc=Select the folder where Star Wars: Battlefront II (Classic, 2005) is installed.
...
begin
  DefaultResult := ExpandConstant('{pf32}\Steam\steamapps\common\Star Wars Battlefront II Classic');
  ...
  begin
    ...
    if Path <> '' then
    begin
      Path := AddBackslash(Path) + 'steamapps\common\Star Wars Battlefront II Classic';
      if IsValidWindowsPath(Path) then
      begin
...
begin
  ...
  begin
    if not FileExists(ExpandConstant('{app}\GameData\BattlefrontII.exe')) then
    begin
      if MsgBox(FmtMessage(CustomMessage('GameFilesNotFound'), ['BattlefrontII.exe']), mbConfirmation, MB_YESNO) = IDNO then
        Result := False;
    end;
  end;
end;
...
var
  D3D9Path, D3D9BackupPath: String;
begin
  ...
  begin
    ...
    if ActiveLanguage = 'ukrainian' then
      MsgBox('Українізатор успішно видалено.' #13#10#13#10 +
             ...
             '2. Натисніть правою кнопкою миші на STAR WARS™ Battlefront II (Classic, 2005)' #13#10 +
      ...
      MsgBox('The Ukrainian localization has been successfully removed.' #13#10#13#10 +
             ...
             '1. Open Steam' #13#10 +
             '2. Right-click on STAR WARS™ Battlefront II (Classic, 2005)' #13#10 +
